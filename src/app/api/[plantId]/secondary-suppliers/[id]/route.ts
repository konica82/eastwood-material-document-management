...
import { PatchSecondarySupplierSchema } from "@/lib/schemas/supplier";
...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    const data = await getServerRepository("supplier", plantId).getSecondary(plantId, id);
    if (!data) return apiError("NOT_FOUND", `Không tìm thấy nhà cung cấp phụ ${id}.`, 404);
    return ok(data);
  } catch (err) {
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    const parsed = PatchSecondarySupplierSchema.safeParse(await req.json());
    if (!parsed.success) return validationError(parsed.error.issues[0]?.message ?? 'Dữ liệu không hợp lệ');

    const data = await getServerRepository("supplier", plantId).updateSecondary(plantId, id, parsed.data);
    return ok(data);
  } catch (err) {
    return handleError(err);
  }
