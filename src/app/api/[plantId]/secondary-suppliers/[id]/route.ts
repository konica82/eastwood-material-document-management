...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    const data = await getServerRepository("supplier").getSecondary(plantId, id);
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    const data = await getServerRepository("supplier").updateSecondary(plantId, id, parsed.data);
    return ok(data);
  } catch (err) {
    return handleError(err);
  }
