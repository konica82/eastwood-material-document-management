...
import { PatchMaterialSchema } from "@/lib/schemas/material";
...
export async function GET(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    const data = await getServerRepository("material", plantId).get(plantId, id);
    if (!data) return apiError("NOT_FOUND", `Không tìm thấy nguyên liệu ${id}.`, 404);
    return ok(data);
  } catch (err) {
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    const parsed = PatchMaterialSchema.safeParse(await req.json());
    if (!parsed.success) return validationError(parsed.error.issues[0]?.message ?? 'Dữ liệu không hợp lệ');

    const data = await getServerRepository("material", plantId).update(plantId, id, parsed.data);
    return ok(data);
  } catch (err) {
    return handleError(err);
  }
