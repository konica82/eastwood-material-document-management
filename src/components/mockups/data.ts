...
export interface MockStatusDef {
  id: string;
  label: string;
  tone: string;
}

export interface MockCargoRow {
  id: string;
  plate: string;
  driver: string;
  driverId: string;
...
export interface MockSecondary {
  name: string;
  id: string;
  share: number;
  plots: number;
...
export interface MockSupplierDetail {
  ...
  registered: Date;
  status: string;
  secondaries: MockSecondary[];
}

export interface MockTopSupplier {
  name: string;
  short: string;
  kg: number;
  count: number;
...
export interface MockMaterialStatBase {
  name: string;
  kg30: number;
  count30: number;
  delta: number;
...
export interface MockMaterialStat extends MockMaterialStatBase {
  kg: number;
  count: number;
}

export type MockDailyIntake = Record<string, number | Date>;
...
export const NAV_ITEMS: MockNavItem[] = [
...
export const STATUS: Record<string, MockStatusDef> = {
...
export const MATERIALS = ['Keo lai', 'Cao su', 'Điều', 'Tràm nước', 'Bạch đàn', 'Mùn cưa', 'Dăm gỗ keo'];

export const SUPPLIERS_BRIEF = [
...
// ─── Material palette ─────────────────────────────────────────────────────────
// NOTE: '#059669', '#0891B2', '#7C3AED', '#BE185D' are chart-only colors.
// TODO: add --chart-color-* primitive tokens for these four values.

export const MATERIAL_PALETTE: Record<string, string> = {
  'Keo lai':    'var(--color-accent)',
  'Cao su':     '#059669',
  'Điều':       'var(--color-warning)',
  'Tràm nước':  '#0891B2',
  'Bạch đàn':   '#7C3AED',
  'Mùn cưa':    '#BE185D',
  'Dăm gỗ keo': 'var(--color-text-secondary)',
};
...
export const MATERIAL_STATS_BASE: MockMaterialStatBase[] = [
...
export const MATERIAL_STATS: Record<string, MockMaterialStat[]> = {
...
export const DAILY_INTAKE_14D: MockDailyIntake[] = (() => {
  const out: MockDailyIntake[] = [];
  for (let d = 13; d >= 0; d--) {
    const dayFactor = 0.70 + (Math.sin(d * 0.5) + 1) / 2 * 0.55;
    const date = new Date(Date.now() - d * 86400000);
    const dow = date.getDay();
    const weekend = (dow === 0 || dow === 6) ? 0.55 : 1;
    const day: MockDailyIntake = { day: d, date };
...
export const TOP_SUPPLIERS: MockTopSupplier[] = [
...
export const RISK: Record<'low' | 'medium' | 'high', MockRiskDef> = {
  low:    { label: 'Rủi ro thấp', color: 'var(--color-success)' },
  medium: { label: 'Rủi ro vừa',  color: 'var(--color-warning)' },
  high:   { label: 'Rủi ro cao',  color: 'var(--color-danger)'  },
};

// ─── Suppliers ────────────────────────────────────────────────────────────────

export const SUPPLIER_DETAIL: MockSupplierDetail = {
