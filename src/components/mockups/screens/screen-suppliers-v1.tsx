...
// Suppliers v1 — primary detail with secondaries
// Reference mockup only — not a routed page.

import React, { useState } from 'react';
import { SUPPLIER_DETAIL, fmtDate } from '../data';
import { Icon, Badge, Button, TextInput, Card, Th, Td } from '../ui';
...
interface ScreenSuppliersV1Props {
...
export function ScreenSuppliersV1({ onToast }: ScreenSuppliersV1Props) {
  const s = SUPPLIER_DETAIL;
  const [secs, setSecs] = useState<Secondary[]>(
    s.secondaries.map(x => ({ name: x.name, id: x.id, share: x.share, plots: x.plots }))
  );
  ...
  return (
    <div data-screen-label="suppliers-v1" style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 320px', minWidth: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 24px', marginTop: 16, fontSize: 13 }}>
              <KV label="Mã số thuế" value={<span style={{ fontFamily: 'var(--font-mono)' }}>{s.taxCode}</span>} />
              <KV label="Người đại diện" value={s.rep} />
              <KV label="Số điện thoại" value={<span style={{ fontFamily: 'var(--font-mono)' }}>{s.phone}</span>} />
              <KV label="Ngày đăng ký" value={<span style={{ fontFamily: 'var(--font-mono)' }}>{fmtDate(s.registered)}</span>} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12 }}>
            ...
            <div style={{
              ...
              <MiniStat label="NCC phụ" value={String(secs.length)} />
            </div>
          </div>
        </div>
      </Card>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 2 }}>
              Các hộ rừng cá nhân thuộc hợp tác xã · tổng cổ phần{' '}
              <span style={{ fontFamily: 'var(--font-mono)' }}>{totalShare}%</span>
            </div>
          </div>
          ...
          <tbody>
            {secs.map((row, i) => (
              <tr key={i}>
                ...
                <Td align="right">
                  <span style={{ fontFamily: 'var(--font-mono)' }}>{row.plots}</span>
                </Td>
                <Td align="right">
            ...
            <tr>
              <td colSpan={5} style={{
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <TextInput value={newId} onChange={setNewId} placeholder="Số CCCD (12 số)" mono style={{ width: 200 }} />
...
function MiniStat({ label, value }: MiniStatProps) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 16, fontWeight: 500, color: 'var(--color-text-primary)', marginTop: 2 }}>{value}</div>
    </div>
  );
}
...
function ShareBar({ value }: ShareBarProps) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, width: 100, justifyContent: 'flex-end' }}>
      ...
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--color-text-primary)', width: 32, textAlign: 'right' }}>
        {value}%
      </span>
    </div>
  );
